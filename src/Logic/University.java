...
public class University {
	...
	public ArrayList<Access> getAccess(Person person) {
		...
		case "Technical":	
			...
			ad.getClassification().add("Local de servidores");
	...
	public ArrayList<ArrayList<AuxVisitOutHour>> visitOutOfTime(String typePerson){
		...
		for (Office o: computerFac.getOffices()) {
			...
			for (Register r: o.getRegister()) {
				...
				if (typePerson.equals(nameClass)) {
					...
					for (int i=0;i<a.size()&&!found;i++) {
						acc=a.get(i);			
						for (int j=0;j<acc.getClassification().size()&&!found;j++) {
							String clas=acc.getClassification().get(j);
							if (clas.equals(o.getClassification())||clas.equals("*")) {
								found=true;
							}
						}	
					}
					if (found) {
						...
						aux.add(new AuxVisitOutHour(r.getPerson(), r.getCheckInDate(), r.getCheckOutDate(), "Sin acceso"));
					}
				}
			}
			persons.add(aux);
			...
			public void automaticData() {
				...
				of1.getRegister().add(new VisitorRegister(new Date(1654164000000L),new Date(1654171200000L),a, "Mecánica", "Controlando", "70022446543")); //fecha de entrada:  2/6/2022-10:00am  || fecha de salida: 2/6/2022-12:00pm
				...
				of3.getRegister().add(new VisitorRegister(new Date(1652871600000L),new Date(1652886000000L),a, "Mecánica", "Controlando", "70022446543")); //fecha de entrada:  18/5/2022-11:00am || fecha de salida: 18/5/2022-15:00pm || excede fecha de salida porq es visitante
